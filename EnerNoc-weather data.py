# -*- coding: utf-8 -*-
"""
Created on Thu May 31 14:15:31 2018
...
#    global site_id
    ...
    with open(str(site_id)+ ' weather.csv','w',newline='') as f:
        w = csv.writer(f)
        w.writerow(['site_id', str(site_id),'lat', float(latitude),'long', float(longitude)])
        w.writerows(data)
# prepping to merge using merge_key
    df1 = pd.read_csv(str(site_id)+ '.csv')
    df1.drop(df1.columns[[0,3,4]], axis=1, inplace=True)
    df1['merge_key'] = df1['dttm_utc']
    new_df1 = df1.assign(UniqueID= str(site_id))
    final_df1 = new_df1.rename(columns={'dttm_utc':'DateTime','value':'Usage_KWH'})
    final_df1 = final_df1[['DateTime','Usage_KWH','UniqueID','merge_key']]
    print(final_df1)
# need to change Datetime into same format as SCE format
    
    df1['merge_key']= pd.to_datetime(df1['merge_key'])
    df1['merge_key']= df1['merge_key'].dt.ceil('30min').astype(str)
#    df1.to_csv(str(site_id)+ ' ready to merge.csv')
    
    df2 = pd.read_csv(str(site_id)+' weather.csv', skiprows=1)
    df2['merge_key'] = df2['Unnamed: 0'].astype(str)
#   df2.to_csv(str(site_id)+ ' almost merge.csv')
    
    df3 = df1.merge(df2)  
#   df3.drop('Unnamed: 0',8)
    df3.to_csv(str(site_id)+ ' merge.csv')
